...
typedef RT_MODEL_part4_prob2_estimato_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * part4_prob2_estimator(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(part4_prob2_estimator_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(part4_prob2_estimator_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: part4_prob2_estimator -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        result = hil_task_start
          (part4_prob2_estimator_DW.HILReadEncoderTimebase_Task, (t_clock)
           part4_prob2_estimator_P.HILReadEncoderTimebase_clock, 500.0, -1)
          ;
        if (result == 0) {
